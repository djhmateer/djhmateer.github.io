...
Ubuntu on the desktop works pretty well, and for certain things (the terminal!) I prefer it to Windows 10.

I see the future direction of Microsoft and my own company to be more linux embracing on as it is more cost effecient to run Linux servers (need fewer resources) than Windows servers and ASP.NET Core runs just fine on both.
