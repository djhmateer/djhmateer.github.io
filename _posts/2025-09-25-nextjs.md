...
# top line from peer to scram-sha-256
...
# Lets try connection from Windows side
# dbeaver gui

# 172.31.112.181 - laptop
hostname -I

# it will fail 

# add at bottom of pg_hba.conf
...
```bash
# see if I need libpq-dev
sudo apt install postgresql postgresql-contrib

sudo service postgresql start
sudo service postgresql start

# create user dave as superuser so I can psql more easily
...
# to Allow External Connections (for pgAdmin)
...
# powershell find IP of WSL2 eg 172.23.16.249
wsl hostname -I
```
...
**HERE** - try install on laptop so I can understand it!
 also change ports on dev machine so the same?
