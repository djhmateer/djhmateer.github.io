...
[From the introduction to SignalR -](https://docs.microsoft.com/en-gb/aspnet/core/signalr/introduction?view=aspnetcore-5.0)SignalR is an open-source library that simplifies adding real-time web functionality to apps. Real-time web functionality enables server-side code to push content to clients instantly.
...
The server library is included in ASP.NET Core 3.0 shared framework, however the JavaScript client library isn't, so lets use Library Manager ([LibMan](https://docs.microsoft.com/en-us/aspnet/core/client-side/libman/?view=aspnetcore-3.0))
